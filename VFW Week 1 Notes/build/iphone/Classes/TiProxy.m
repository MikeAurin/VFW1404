...
//Rare exceptions to indicate a bug in the _vfwweek1notes code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_vfwweek1notes = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_vfwweek1notes]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.vfwweek1notes.module"
