...
NSString * VFWWeek1Notes$ModuleRequireFormat = @"(function(exports){"
...
@implementation VFWWeek1NotesObject
...
{
	// create VFWWeek1Notes global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "VFWWeek1Notes" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_vfwweek1notes = [[VFWWeek1NotesObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_vfwweek1notes];
	
	NSString *_vfwweek1notesNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _vfwweek1notesNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_vfwweek1notes valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:VFWWeek1Notes$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.vfwweek1notes.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.vfwweek1notes.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
